...
const queue = new Map();
const emojiChannelID = "720667292551348235"
/////////////////////////
//const http = require('http');
...
music.on("ready", async () => {
  music.user.setActivity('MNG ONLY', {Type: 'STREAMING', url:'https://www.twitch.tv/xlittlerascal'});
  console.log(`Logged in as : ${music.user.tag}`);
  console.log(`${music.user.username} bot ready!`)

});
//////////////////////
  /*music.user.setActivity('MNG ONLY',
  {Type: 'STREAMING', 
   url:'https://www.twitch.tv/xlittlerascal'});
  console.log(`Logged in as : ${music.user.tag}`);
...
music.on('message', message=> {
    ...
    message.author.sendMessage("**OWNER GANTENG**");
...
music.on("message", message => {
  if (message.content.startsWith('!announce')) {
    ...
    let embed = new Discord.RichEmbed()
      ...
      .setImage("https://i.imgur.com/zYz0lys.jpg")
      .setURL(array_of_arguments[2])
      .setThumbnail("hhttps://i.imgur.com/9l5CZOW.png")
      .setColor("#FF4654")
      .setFooter("MNG ONLY","https://i.imgur.com/9l5CZOW.png")
      .setTimestamp();
...
music.on('message', msg => {
  ...
  if (msg.guild && msg.content.startsWith('v!dmall')) {
    if(msg.author.id !== '642210549974958081') return msg.channel.send(kontol)
...
/*music.on('voiceStateUpdate',(peka, dikit) => {
  ...
  let chan = `Team ${id}`
  var state = null;  
  const kategorikanal = '693873380897849394'
  const channelid = '693873380897849394'
